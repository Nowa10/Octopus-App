...
export default function MatchList({
    ...
    async function ensureLoserRound(r: number) {
        ...
        // ✅ Finale LB : quand r == finale WB, on ne prend QUE winners(LB r-1)
        if (r === wbMax) {
        ...
        if (r === 1) {
            // Croisement R1 : [0,2,1,3] -> (0vs2) (1vs3) pour éviter les re-matchs immédiats
            const even = feed.filter((_, i) => i % 2 === 0);
            const odd = feed.filter((_, i) => i % 2 === 1);
            const reordered = [...even, ...odd];

            for (let i = 0; i + 1 < reordered.length; i += 2) {
                pairs.push([reordered[i]!, reordered[i + 1]!] as [string, string]);
            }
        } else {
            ...
            const k = Math.min(carry.length, feed.length); // ❌ pas de BYE : on tronque
            for (let i = 0; i < k; i++) {
                pairs.push([carry[i]!, feed[i]!] as [string, string]);
            }
        }
        ...
        for (const [a, b] of pairs) {
            await setPlayersIfBoth(r, slot++, a, b, 'loser'); // ⛔ setPlayersIfBoth empêche tout BYE
        }
    }
    ...
    async function rebuildLoserProgression() {
        ...
        // On va jusqu’à wbMax pour générer aussi la finale LB
