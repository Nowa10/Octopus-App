...
function rrPairsAllRounds(ids: (string | null)[]) {
    ...
    for (let r = 0; r < total; r++) {
        ...
        // rotation canonique: fixe index 0, dernier -> position 1
        const fixed = arr[0];
...
/** Répartition en poules équilibrées (vise 4, sinon 3, max 5) + serpentin */
...
/** Anti re-match R1 (swap des seconds si possible) */
...
export default function MatchList({
    ...
    /* ============================
       Décision de format (règles simples & stables)
       - N ≤ 6 : POULE (toujours)
    ...
    // Poule unique (RR) — BYE garanti 1x/joueur (on ne stocke pas les BYE)
    ...
    // Multi-poules (intercalage A1,B1,…, A2,B2, …)
    ...
    const recomputePoolTabs = useCallback(async () => {
        ...
        for (let r = 1; r <= max; r++) {
            ...
            for (const m of ms) { if (m.player1) allIds.add(m.player1); if (m.player2) allIds.add(m.player2); }
            const idx = rosterSets.findIndex((S) => [...allIds].every((id) => S.has(id)));
            if (idx >= 0) { roundsByPool[idx].push(r); poolRoundSet.add(r); }
        }

        const lastPoolRound = poolRoundSet.size ? Math.max(...Array.from(poolRoundSet)) : 0;
    ...
    async function computeQualifiedFromPools(target: 8 | 16) {
        ...
        let seeds: string[] = [];
        if (target === 8) {
            seeds = K >= 4 ? [...firsts, ...seconds].slice(0, 8)
                : [...firsts, ...seconds, ...thirds].slice(0, 8);
        } else {
            seeds = K === 4
                ? [...firsts, ...seconds, ...thirds, ...fourths].slice(0, 16)
                : [...firsts, ...seconds, ...thirds, ...fourths].slice(0, 16);
        }
    ...
    /** Playoffs auto si toutes les poules sont finies (robuste, sans -Infinity) */
    async function ensurePlayoffsIfReady(tId: string) {
        ...
        // Round de départ = max des rounds “poule”
        const used: number[] = roundsByPool.flat().filter((x) => typeof x === 'number' && x > 0);
        if (used.length === 0) return;
        const lastPoolRound = Math.max(...used);
    ...
    async function applyWinner(m: M, winnerId: string) {
        ...
        if (m.bracket_type === 'winner') {
            ...
            if (m.round === max) {
                ...
                const current = (prof?.wins ?? 0) as number;
    ...
    /* ============================
       ...
       ============================ */
    const initialBuild = useCallback(async () => {
        setIsBuilding(true);
        ...
        setIsBuilding(false);
        setSetupDone(true);
    }, [tournamentId, recomputePoolTabs, load]); // ✅ deps complètes

    useEffect(() => {
        if (!setupDone) { void initialBuild(); }
    }, [setupDone, initialBuild]);

    // Quand la liste des matches change : mise à jour des onglets + tentative de playoffs auto
    useEffect(() => {
        (async () => {
            await recomputePoolTabs();
            await ensurePlayoffsIfReady(tournamentId);
        })();
        // pas de reload() ici pour éviter la boucle
    }, [matches, recomputePoolTabs, tournamentId]);
    ...
    async function computePodium() {
        ...
        type MinimalRow = Pick<M, 'winner' | 'status'>;
        const rows: MinimalRow[] = (all ?? []) as MinimalRow[];
        const wins = new Map<string, number>();
    ...
    /* ============================
       Bouton manuel “Générer les playoffs”
       ============================ */
    const manualGeneratePlayoffs = useCallback(async () => {
        setIsBuilding(true);
        await recomputePoolTabs();

        // mêmes gardes que l’auto
        const { roundsByPool } = poolTabs;
        if (!roundsByPool.length) { setIsBuilding(false); return; }
        ...
        if (!allDone) { setIsBuilding(false); return; }

        const used: number[] = roundsByPool.flat().filter((x) => typeof x === 'number' && x > 0);
        if (used.length === 0) { setIsBuilding(false); return; }
        const lastPoolRound = Math.max(...used);

        const next = await fetchRound(tournamentId, 'winner', lastPoolRound + 1);
        if (next.length === 0) {
            const ids = await getParticipantsOrdered(tournamentId);
            const target: 8 | 16 = ids.length <= 12 ? 8 : 16;
            const { seeds, samePool } = await computeQualifiedFromPools(target);
            if (seeds.length >= 4) {
                await buildSEBracketFromSeeds(tournamentId, seeds, lastPoolRound + 1, samePool);
            }
        }
        ...
        await recomputePoolTabs();
        setIsBuilding(false);
    }, [poolTabs, tournamentId, recomputePoolTabs, load]);
            ...
            <div className="hstack">
                ...
                {canEdit && (
                    <div className="hstack" style={{ gap: 8 }}>
                        ...
                        {/* Bouton manuel Playoffs */}
                        {poolTabs.enabled && (
                            <Button variant="ghost" onClick={() => safeAction(() => { void manualGeneratePlayoffs(); })}>
                                    ...
                                    <div className="card__content stack">
                                        ...
                                        {canEdit && !podium && (
                                            <div className="hstack" style={{ marginTop: 8 }}>
                                                <span className="spacer" />
                                                <Button size="sm" variant="danger" onClick={() => safeAction(() => { void reset(m); })}>Réinitialiser</Button>
                                            </div>
                                        )}
                                    </div>
                                </div>
